...
for team in nba_teams:
    ...
    for game in home_games:
        ...
        for item in playersWithStats:
            if (item['name'] == playerOne_fullname):
                for x in f:
                    first_name = x.split()[0]
                    last_name = x.split()[1]
                    name = first_name + ' ' + last_name
                    if (name in playerTwo_fullname):
                        item['total_wp'] += float(x.split()[len(x.split())-1])
                        item['count'] += 1
                        item['o_wp'] = item['total_wp'] / item['count']

                        print(item['name'], end=' ')
                        print(item['total_wp'], end=' ')
        ...
        f = open("player_data.txt", "r")
        for item in playersWithStats:
            if (item['name'] == playerTwo_fullname):
                for x in f:
                    first_name = x.split()[0]
                    last_name = x.split()[1]
                    name = first_name + ' ' + last_name
                    if (name in playerOne_fullname):
                        item['total_wp'] += float(x.split()[len(x.split())-1])
                        item['count'] += 1
                        item['o_wp'] = item['total_wp'] / item['count']
...
f = open("player_owp.txt", "w")
for item in playersWithStats:
    if (item['count'] > 0):
        f.write(str(item['name']))
        f.write(' ')
        f.write(str(item['total_wp']))
        f.write(' ')
        f.write(str(item['count']))
        f.write(' ')
        f.write(str(item['o_wp']))
        f.write('\n')
